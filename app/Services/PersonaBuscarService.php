...
class PersonaBuscarService
{
    public static function ejecutar($request)
    {
        ...
        //  límite sano (evita que te pidan 10000 registros)
        $porPagina = min($request->input('per_page', 15), 100);
        ...
        //  IMPORTANTE: esto usa automáticamente ?page=...
    ...
    if ($anio || $idFacultad || $idClaustro) {
        $query->whereHas('inscripciones', function ($q) use ($anio, $idFacultad, $idClaustro) {
            ...
            $q->whereNull('deleted_at'); // importante también acá
